...
def tallenna(info):
    try:
        with open("tulokset.json", "w") as k:
            json.dump(info, k)
    except (IOError, json.JSONDecodeError):
        print("Tiedoston avaaminen ei onnistunut.")
...
def kysy_luku(kysymys, raja, virhe):
    while True:
        try:
...
def setup():
    ...
    Asettaa asetukset.
    """
    korkeus = kysy_luku("    Anna kentän korkeus: ", 26, "    Liian korkea kenttä!")
    
    leveys = kysy_luku("    Anna kentän leveys: ", 46, "    Liian leveä kenttä!")
    
    miinat = kysy_luku("    Anna miinojen määrä: ", int(korkeus * leveys * 4 / 5), "    Liian paljon miinoja!")
