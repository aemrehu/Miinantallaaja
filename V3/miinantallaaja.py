...
def tulvataytto(nakyva, kentta, x, y):
    """
            ...
            for j in [b-1, b, b+1]:
                for i in [a-1, a, a+1]:
                    if j < 0 or i < 0:
                        ...
                        pass
                    elif kentta[b][a] in ("1", "2", "3", "4", "5", "6", "7", "8"):
                        break
                    else:
                        try:
                            if kentta[j][i] == " ":
...
def kasittele_hiiri(x, y, nappi, muokkaus):
    ...
    if asetukset["voitto"] == 1 or asetukset["havio"] == 1:
        if nappi in (haravasto.HIIRI_VASEN, haravasto.HIIRI_OIKEA):
            lopeta()
    else:
        if nappi == haravasto.HIIRI_VASEN:
            c, v = int(x/40), int(y/40)
            tulvataytto(tila["nakyva"], tila["kentta"], c, v)
...
def main():
    ...
    haravasto.aseta_toistuva_kasittelija(paivita_peli, 1/8)
